...
impl<'a> DisplayBuild for &'a mut sdl2::video::WindowBuilder {
    ...
    type Err = glium::GliumCreationError<WindowBuildError>;

    fn build_glium_debug(self, debug: debug::DebugCallbackBehavior)
                         -> Result<SDL2Facade, glium::GliumCreationError<WindowBuildError>> {
    ...
    unsafe fn build_glium_unchecked_debug(
        self, debug: debug::DebugCallbackBehavior)
        -> Result<SDL2Facade, glium::GliumCreationError<WindowBuildError>>
    {
        let backend = Rc::new(try!(SDL2WindowBackend::new(self)));
...
impl SDL2WindowBackend {
    ...
    pub fn new(window_builder: &mut sdl2::video::WindowBuilder)
               -> Result<SDL2WindowBackend,WindowBuildError> {
        let window = try!(window_builder.opengl().build());
        let context = try!(window.gl_create_context()
                           .map_err(WindowBuildError::SdlError));
